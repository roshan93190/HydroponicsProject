...
public class GlobalExceptionHandler {
	...
	public ResponseEntity<MyErrorDetails> mynotFoundHandler(NoHandlerFoundException nfe, WebRequest req) {
		MyErrorDetails err = new MyErrorDetails(LocalDateTime.now(), "Rest Handler Not Found (check for valid URI)", req.getDescription(false));
		return new ResponseEntity<>(err, HttpStatus.NOT_FOUND);
	...
	public ResponseEntity<MyErrorDetails> handleValidationException(Exception exp, WebRequest req) {
		MyErrorDetails err = new MyErrorDetails(LocalDateTime.now(),
				exp.getMessage(), req.getDescription(false));
